...
(6 встроенных демо). JSON — в stdout, ошибки — в stderr, код выхода ≠ 0 при ошибке.
...
./gradlew :tools:agent-console:run --args="render --project /path/proj --screen missionTelemetry --out /tmp/t.png"
...
./gradlew :tools:agent-console:run --args="inspect --project /path/proj --screen missionTelemetry --node t_tile_1"

# Диагностика: компиляция + validateDesignDocument; exit 2 при error-severity
./gradlew :tools:agent-console:run --args="validate --project /path/proj --screen missionTelemetry"

# Выгрузить встроенные демо как стартовый корпус файлов
