...
### 7.1b. 🔴 Решение §P0.7 («label на типизированном payload'е → `diagnostics.error`») НЕ ВЫЖИЛО

**Регрессия, найденная пользователем:** локальная папка-проект перестала открываться со
стартового экрана, **без единой ошибки на экране**.

Механизм: загрузчик **compile-gate'ит по severity**. `MissionEditorScreen.onFolderSnapshot`
делает `docs.document?.takeUnless { docs.hasErrors }` и при `null` — `folderExternalError = true;
return` («удержать последнюю хорошую канву»). На стартовом экране удерживать нечего → клик молча
не делает ничего. Индикатор `folderExternalError` рендерится только внутри
`EditorSourcePane.kt:578`, которого на стартовом экране нет — отсюда «нет никаких ошибок».

Ирония: осиротевшие `label` — это то, что писал **старый сломанный inline-редактор**. То есть
исправление бага «кирпичило» ровно те документы, которые этот баг и испортил.

План формулировал это как «desired (диагностика, не падение)» — при живом compile-gate это
именно **падение**. Итог: **warning**, узел и label сохраняются (текст может быть тем, что автор
и хотел видеть), документ грузится. Регресс-тест
`aDocumentCarryingAnOrphanedLabelStillLoads` красный на отгруженном ридере, зелёный сейчас.

Смежный (НЕ мой, не чинился) UX-пробел: **любая** ошибка компиляции внешней папки = молчаливый
no-op на стартовом экране. Стоит показывать причину прямо там.
